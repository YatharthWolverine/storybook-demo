...
export const TeamSlider = ({ apidata, left_arrow_img, right_arrow_img }) => {
            ...
            <Slider {...settings} ref={customeSlider}>
              {apidata?.map((item, index) => (
                <ProfileCard
                  key={index}
                  isShowIcons
                  profileUrl={`/team/${item?.slug}`}
                  twiterUrl={item?.social_media_icon?.[0]?.link}
                  linkedInUrl={item?.social_media_icon?.[1]?.link}
